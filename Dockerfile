...
RUN apt-get update -y && apt-get install make git gettext gnupg2 -y && \
    ...
    echo "deb http://packages.inverse.ca/SOGo/nightly/5/debian/ buster buster" >> /etc/apt/sources.list && \
    apt-get update && \
...
RUN curl -Lo- "https://raw.githubusercontent.com/shelltips/bpkg/master/setup.sh" | bash
#RUN PATH=$PATH:/root/.local/bin /root/.local/bin/bpkg install cha87de/bashutil -g
